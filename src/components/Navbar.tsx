...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Get hero section height (assuming it takes up the full viewport height)
      const heroHeight = window.innerHeight;
      setIsAfterHero(window.scrollY > heroHeight - 64); // 64px is navbar height
    };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex-shrink-0">
            <div className={`${isAfterHero ? 'bg-primary text-white' : 'bg-white/10 text-white'} px-6 py-2 rounded`}>
              Logo
            </div>
          </div>
